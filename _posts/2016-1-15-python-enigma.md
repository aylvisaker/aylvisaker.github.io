...
Now this would all be quite over-elaborate if the rotors didn't also move. When the wheels rotate, they effectively scramble the entire circuit. So if Alice sets her machine to `ABC`  (indicating the starting configuration of the rotors) and types `HELLO WORLD` she might read something like `IOVDA YUEBH`. Notice that the letter `L` got mapped to a different letter each time it appeared in the message! Cryptographers call that a [polyalphabetic cipher](http://wki.pe/Polyalphabetic_cipher). 
